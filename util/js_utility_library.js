...
(function() {
   ...
   */
   ...
   function toNumber(numberToConvert) {
     // Let's just throw an exception if the user tries to pass in something that
     // is not a number, better to know now as opposed to returning NaN
     var numToReturn = parseInt(numberToConvert);

     if (isNaN(numToReturn)) {
      throw {message : 'You tried to convert ' + numberToConvert + ' to a number, but it is NaN.'};
     } else {
  ...
  /**
   * This function will insert into an array at the index we pass in.
   *
   * @param {Object.Array} arrayToInsertInto The array we want to insert into.
   * @param {Number}       indexToInsertAt   The index of the array we want to insert at.
   * @param {Number}       valueToInsert     The value we will be inserting into the array.
   *
   * @return {Object.Array} The array with the new number inserted into it. 
   */
  function insert(arrayToInsertInto, indexToInsertAt, valueToInsert) {
    return arrayToInsertInto.splice(indexToInsertAt, 0, valueToInsert);
  }
